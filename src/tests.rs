...
mod tests {
    ...
    lazy_static! {
        pub static ref TESTLOG_FILENAME_ARG_STR: String =
            "--log-file=".to_string() + TESTLOG_FILENAME;
    }
    ...
    fn single_test(
        ...
        if lines.len() == 0 {
            panic!(
                "Test{} file error - empty test #{} on line {}",
                match is_logfile {
                    true => "log",
                    false => "",
                },
        ...
        let cmd_res = match run_cmd.output() {
            ...
            Err(e) => {
                panic!(
                    "Can't execute command {} {} (test{} #{} on line {}):\n{}",
                    test_command,
                    lines[0],
                    match is_logfile {
                        true => "log",
        ...
        let out_strings: Vec<String> = match is_logfile {
            false => {
                ...
                let cmd_stdout = match str::from_utf8(&cmd_res.stdout) {
                    Ok(s) => s,
                    Err(e) => panic!(
                        "Invalid UTF-8 sequence in command {} {} output (test #{} on line {}):\n{}",
        ...
        for i in 0..cmd_out_strings.len() {
            print!(
                "\"{}\" - \"{}\"...",
                cmd_out_strings[i], test_out_strings[i]
            );
            ...
            let test_str =
                escape(&rg.replace_all(test_out_strings[i], "@")).replace("@", "(-?\\d+)");
            let test_rg = Regex::new(&test_str).unwrap();

            if !test_rg.is_match(cmd_out_strings[i]) {
                println!("   Error!");
                panic!(
                    "Failed test{} #{} (line {}) - template \"{}\" doesn't match output \"{}\"",
                    match is_logfile {
                        true => "log",
                        false => "",
                    },
