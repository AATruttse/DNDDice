...
use crate::strings::{ADVDISADV_ADV_CODE, ADVDISADV_DISADV_CODE, ADVDISADV_ERROR_MSG};
...
pub fn render_roll(
    ...
    if OPT.is_debug()
        || OPT.verbose > 1
        || force_render
        || (!is_several_rolls && !is_advantage && (!OPT.is_collect_stat() || OPT.verbose > 0))
    {
        let roll_res = format!(
            "{}{}",
            match show_dice_code && !OPT.numbers_only {
                true => ": ",
                _ => "",
            },
            res
...
pub fn format_dice_str(
    ...
    format!(
        "{}{}d{}{}{}{}{}{}",
        match is_several_rolls {
            true => &OPT.tab_delimiter,
            _ => "",
        },
        n,
        d,
...
pub fn render_codes(dices_num: usize, is_advantage: bool, dicecode: &str, res: IntValue) {
    if dices_num == 1 && is_advantage {
        ...
        || (dices_num > 1
            && (OPT.verbose > 0 || (OPT.verbose == 0 && !OPT.is_collect_stat() && dices_num > 1)))
    {
        if !OPT.numbers_only && OPT.verbose > 0 {
            let dicecode_str = format!("{}: ", dicecode);
            output(&dicecode_str);
